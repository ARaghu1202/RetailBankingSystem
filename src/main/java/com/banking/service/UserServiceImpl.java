...
public class UserServiceImpl implements UserService {
	...
	public RegisterResponseDto registerUser(RegisterRequestDto userRegisterDto) {
		...
		if (!optioanlUser.isPresent()) {
			...
			userAccount.setMinimumBalance(ApplicationConstants.ACCOUNT_MINIMUM_BALANCE);
			userAccount.setUserId(user.getId());
			userAccount.setAccountType(ApplicationConstants.ACCOUNT_TYPE);
			userAccount.setBalanceAmount(ApplicationConstants.ACCOUNT_BALANCE_AMOUNT);
			...
			registerResponseDto.setStatus(ApplicationConstants.SUCCESS);
			registerResponseDto.setMessage(ApplicationConstants.REGISTER_SUCCESS_MESSAGE);

		} else {
			throw new UserAlreadyExist(ApplicationConstants.USER_EXIST);
		}
