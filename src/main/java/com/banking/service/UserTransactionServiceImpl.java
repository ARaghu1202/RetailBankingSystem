...
/**
 * UserTransactionServiceImpl - to implement all the transaction operations on UserAccount entity
...
public class UserTransactionServiceImpl implements UserTransactionService {
	...
	/**
	 * fundTransfer-to transfer fund to one user to another user
	...
	public ResponseDto fundTransfer(FundTransferRequestDto fundTransferRequestDto) throws NotFoundException {
		...
		if (userAccountDetail.isPresent() && userPayeeAccountDetail.isPresent()) {
			...
			if (totalAmount <= userAccount.getBalanceAmount()) {
				...
				fundTransferResponseDto.setStatus(ApplicationConstants.SUCCESS);
				fundTransferResponseDto.setMessage(ApplicationConstants.FUND_TRANSFER_SUCCESS);
			} else {
				fundTransferResponseDto.setStatus(ApplicationConstants.FAILURE);
				fundTransferResponseDto.setMessage(ApplicationConstants.FUND_TRANSFER_MIN_BAL);
			}
		} else {
	...
	public String getTransactionNumber() {
		...
		String transactionNumber = ApplicationConstants.GET_TRANSACTION_NO_PREFIX + transactionId;
	...
	private UserTransactionResponseDto convertUserTransactionEntityToUserTransactionResponseDto(
		...
		if (null != userTransactionResponse && userTransactionResponse.size() > ApplicationConstants.ZERO) {
			...
			userTransactionResponseDto.setMessage(ApplicationConstants.OPERATION_SUCCESS);
			...
			userTransactionResponseDto = new UserTransactionResponseDto();
			userTransactionResponseDto.setMessage(ApplicationConstants.NO_RECORD_FOUND);
